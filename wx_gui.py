...
class PaintFrame(wx.Frame):
    ...
    def OnPaintMotion(self, event):
        ...
        # # 设置状态栏1内容
        # self.statusbar.SetStatusText(u"鼠标位置：" + str(event.GetPosition()), 0)
        #
        # # 设置状态栏2内容
    ...
    def CreateMenu(self, menuData):
        ...
        for eachItem in menuData:
            if len(eachItem) == 2:
                ...
                menu.Append(wx.NewId(), label, subMenu)  # 递归创建菜单项
            else:
    ...
    def OnOpen(self, event):
        ...
        打开开文件对话框
        '''
        file_wildcard = "Paint files(*.json)|*.json|All files(*.*)|*.*"
    ...
    def OnSaveAs(self, event):
        ...
        dlg = wx.FileDialog(self,
                            "Save paint as ...",
                            os.getcwd(),
                            style=wx.FC_SAVE | wx.FD_OVERWRITE_PROMPT,
                            wildcard=file_wildcard)
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def ReadFile(self):
        if self.filename:
            try:
                ...
                wx.MessageBox("%s is not a paint file." % self.filename, "error tip",
